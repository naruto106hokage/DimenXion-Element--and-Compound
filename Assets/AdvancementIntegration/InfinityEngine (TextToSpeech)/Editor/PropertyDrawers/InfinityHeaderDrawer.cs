...
namespace InfinityEditor.PropertyDrawers
{
    ...
    public class InfinityHeaderDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                EditorGUI.HelpBox(position, string.Format("The name of the field {0} must be {1}" , property.name, expectedPropertyName), MessageType.Error);
                return;
            }
            if(property.type != "bool")
            {
                EditorGUI.HelpBox(position, string.Format("The type of the field {0} must be a boolean", property.name), MessageType.Error);
                return;
            }
