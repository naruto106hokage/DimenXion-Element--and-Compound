...
[Tooltip("If true then Pointer distance will remain at its last distance if no target is present.")]
public bool MaintainDistanceOnExit = false;

[Tooltip("If true then Pointer distance from camera will not change.")]
public static bool LockDistance = false;
  ...
  void Update() {
		if (isHittingTarget || MaintainDistanceOnExit || LockDistance) {
      return;
    }
