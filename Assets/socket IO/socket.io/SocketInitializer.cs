	...
	public class SocketInitializer : MonoBehaviour
	{
		...
		public string WebSocketUrl {
			get {
				...
				if (BaseUrl.StartsWith ("https://"))
					builder = new StringBuilder (BaseUrl.Replace ("https://", "wss://"));
				else
					builder = new StringBuilder (BaseUrl.Replace ("http://", "ws://"));
		...
		IEnumerator InitCore (UniRx.IObserver<Socket> observer, UniRx.CancellationToken cancelToken)
		{
			...
			if (webSocketTrigger == null || !webSocketTrigger.IsConnected) {
				...
				//Debug.Log ("Converting to json " + www.text);
				...
				if (textIndex != -1) {
					...
					if (lastIndex + 2 <= json.Length)
						json = json.Remove (lastIndex + 1);

					//Debug.Log ("Converting to json " + json);
			...
			webSocketTrigger.OnRecvAsObservable ()
                .Subscribe (r => {
				capturedSocket.OnRecvWebSocketPacket (r);
			})
                .AddTo (Socket);
